 ...
 *                Initialy built by build_class_from_table on 2020-07-16 10:01
 */
...
class BasketMatch extends CommonObject
{
	...
	function create($user, $notrigger = 0)
	{
		...
		$sql .= ' ' . (empty($this->categ) ? 'NULL' : "'" . $this->db->escape($this->categ) . "'") . '';
	...
	function delete($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (!$resql) {
				$error++;
				$this->errors[] = "Error " . $this->db->lasterror();
			} else if ($this->db->affected_rows($resql) == 0) {
				$error++;
				$this->errors[] = "Item no found in database";
			}
