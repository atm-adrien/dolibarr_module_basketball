 ...
 */
class modBasket extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->numero = 500001; // Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve an id number for your module
		// Key text used to identify module (for permissions, menus, etc...)
		$this->rights_class = 'basket';
		...
		$this->description = "basketDescription";
		...
		//$this->automatic_activation = array('FR'=>'basketWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		...
		if (!isset($conf->basket) || !isset($conf->basket->enabled)) {
			$conf->basket = new stdClass();
			$conf->basket->enabled = 0;
		}

		// Array to add new pages in new tabs
		$this->tabs = array('objecttype:+test1:Match:$user->rights->basket->read:/basket/mapagetab1.php?id=__ID__');
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@basket:$user->rights->basket->read:/basket/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		// $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@basket:$user->rights->othermodule->read:/basket/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		...
		$this->dictionaries=array(
			...
			c
			// Condition to show each dictionary
			'tabcond'=>array($conf->basket->enabled, $conf->basket->enabled, $conf->basket->enabled)
		);
		*/

		// Boxes/Widgets
		// Add here list of php file(s) stored in basket/core/boxes that contains a class to show a widget.
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'basketwidget1.php@basket',
			//      'note' => 'Widget provided by basket',
		...
		$this->cronjobs = array(
			...
			//      'label' => 'MyJob label',
			//      'jobtype' => 'method',
			//      'class' => '/basket/class/myobject.class.php',
			//      'objectname' => 'MyObject',
		...
		$this->rights[$r][1] = 'Read objects of basket'; // Permission label
		$this->rights[$r][4] = 'basketmatch'; // In php code, permission will be checked by test if ($user->rights->basket->level1->level2)
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->basket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero . $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update objects of basket'; // Permission label
		$this->rights[$r][4] = 'basketmatch'; // In php code, permission will be checked by test if ($user->rights->basket->level1->level2)
		$this->rights[$r][5] = 'create'; // In php code, permission will be checked by test if ($user->rights->basket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero . $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete objects of basket'; // Permission label
		$this->rights[$r][4] = 'basketmatch'; // In php code, permission will be checked by test if ($user->rights->basket->level1->level2)
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/basket/sql/');
		if ($result < 0) return -1; // Do not activate module if error 'not allowed' returned when loading module SQL queries (the _load_table run sql with run_sql with the error allowed parameter set to 'default')

		// Create extrafields during init
		include_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
		$extrafields = new ExtraFields($this->db);
		$result1=$extrafields->addExtraField('MentSpec', ''.$langs->trans('SpecificMention'), 'text', 1,  3, 'propal',   0, 0, '', '', 1, '', 1, 0, '', '', 'basket@basket', '$conf->basket->enabled', 0, 1);
